...
double GeometriaHelper::CalculatePitch(UInt_t pl, double phi, double theta) const
{
  ...
  if (geom->PlaneToView(pl) == galleryfmwk::geo::kUnknown ||
    ...
    galleryfmwk::Message::send(__FUNCTION__, Form("Warning :  no Pitch foreseen for view %d", geom->PlaneToView(pl)));
    return pitch;
  }
  else if ( pl >= geom->Nplanes() ) {
    galleryfmwk::Message::send(__FUNCTION__, "Plane number larger than max. number of planes");
    return pitch;
  }
  else {
